...
class Client {
	...
	public function read($path, $eloquentCollection = false) {
		...
		$response = $this->validateResponse($request)->getBody()->getContents();
		$result = json_decode($response, 1);

		// Is Response Valid?
		return ($eloquentCollection) ? $this->makeCollection($response, $eloquentCollection) : new Collection($result);
	}
